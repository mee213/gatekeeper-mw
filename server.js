...
function gateKeeper(req, res, next) {
  ...
  // look for a header called 'x-username-and-password' and parse the value
  const parsed = queryString.parse(req.get('x-username-and-password'));
  
  // search the array of USERS for a match to the user-submitted username
  const matchedUser = USERS.find(function(user) {
    return user.userName === parsed.user;
  });
  
  // set req.user to the matchedUSER (will remain undefined if no match found)
